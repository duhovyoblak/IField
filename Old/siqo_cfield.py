...
class ComplexField:
    ...
    def __init__(self, name):
        ...
        self.logger.debug(f"ComplexField.constructor: {name}")
    ...
    def gener(self, dimName, count, offMin, offMax, offType=_LIN, origPos=[]):
        ...
        #----------------------------------------------------------------------
        offs   = ComplexField.genOffset(count, self.offMin, self.offMax, self.offType)
        actPos = list(origPos)
        actPos.append(0)
    ...
    def extend(self, dimName, count, offMin=None, offMax=None, offType=_LIN):
        ...
        for node in self:
            ...
            subField = ComplexField(f"{self.name}_sub")
